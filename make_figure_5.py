...
def get_confidence_intervals(rec_goal_maps, rec_weights, sigma, gamma):
    ...
    train_indices = np.load(GEN_DIR_NAME + 'unrestricted_train_indices.npy').tolist()
...
def get_val_lls_to_plot():
    ...
    sigmas_best = [0.15, 0.08, 0.125, 0.25]
    lr_maps_best = [0.05, 0.01, 0.05, 0.05]
    lr_weights_best = [0.005, 0.005, 0.005, 0.005]
    gammas_best = [0.99, 0.99, 0.9, 0.7]
    lams_best = [0.001, 0.001, 0.001, 0.001]
    seeds_best = [2,2,1,1]

    for n, n_maps in enumerate(N_MAPS):
        load_dir = REC_DIR_NAME + 'maps_' + str(n_maps) + '_sigma_' + str(sigmas_best[n]) + \
                   '_lr_' + str(lr_maps_best[n]) + '_' + str(lr_weights_best[n]) + \
                   '_' + str(gammas_best[n]) + '_' + str(lams_best[n]) + '/'
        val_ll_this_setting = np.load(load_dir + 'validation_lls_' + str(N_TRAJS) + \
                                      '_seed_' + str(seeds_best[n]) + '_iters_100.npy')[-1]
...
if __name__=="__main__":
    ...
    TRAJS_DIR_NAME = GEN_DIR_NAME + 'water_unrestricted_mice_trajs.pickle'
    REC_DIR_NAME = 'recovered_parameters/mice_recovered_params/water_unrestricted/' # directory that should store parameters recovered by DIRL
    N_TRAJS = 207 # num of trajectories of water-unrestricted mice
    SAVE_DIR = 'figures/' # directory to save figures in
    if not os.path.exists(SAVE_DIR):
        os.makedirs(SAVE_DIR)

    # parameters to reproduce Fig 5
    sigma = 0.08 # noise variance of time-varying weights
    lr_maps = 0.01 # learning rate for goal maps
    lr_weights = 0.005 # learning rate for weights
    lamda = 0.001 # l2 coefficient for goal maps
    n_maps = 2 # number of goal maps
    gamma = 0.99 # discount factor in value iteration
    seed = 2 # initialization seed
    ...
    if TRAIN_DIRL_NOW:
        ...
        fit_dirl_mice(False, lr_weights, lr_maps, max_iters, n_maps, sigma, gamma, lamda, seed, GEN_DIR_NAME, REC_DIR_NAME)
    ...
    MAP_LABELS = ["explore", "home"] # labels of goal maps
    ...
    plt.axis('off')
    fig.savefig(SAVE_DIR + "unrestricted_goal_maps_1.pdf")
    ...
    PlotMazeFunction(converted_map, maze, mode='nodes', numcol=None, figsize=6, axes=ax,)  # , axes= axs[i//5][i%5])
    plt.title('\"explore\"', fontsize=10)
    plt.axis('off')
    fig.savefig(SAVE_DIR + "unrestricted_goal_maps_2.pdf")
    ...
    plt.axhline(y=0,
                ...
                linewidth=0.5)
    plt.ylim(-5, 5)
    plt.yticks([-5, 0, 5], ['','',''],fontsize=10)
    plt.xticks([0, 10], fontsize=10)
    ...
    plt.gca().spines['top'].set_visible(False)
    plt.show()
    fig.savefig(SAVE_DIR + "unrestricted_time_varying_weights.pdf")
    ...
    plt.gca().spines['top'].set_visible(False)
    plt.show()
    fig.savefig(SAVE_DIR + "unrestricted_time_varying_rewards.pdf")
    ...
    max_ent_val_ll = np.load('recovered_parameters/existing_irl_methods_results/maxent_best_ll_unrestricted.npy')
    # load val LL when using DeepMaxEnt
    deep_max_ent_val_ll = np.load('recovered_parameters/existing_irl_methods_results/deepmaxent_best_ll_unrestricted.npy')
    ...
    plt.gca().spines['top'].set_visible(False)
    fig.savefig(SAVE_DIR + "unrestricted_model_comparison.pdf")
    plt.show()